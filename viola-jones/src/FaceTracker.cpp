...
void FaceTracker::trackFaces(vector<Face>& blobs) {
	...
	if(bs) {
		...
		for(int j=0; j<fs; j++) {
			...
			if(bmi(i) == j) {
				...
				faces[j].rect.height = blobs[i].rect.height*1.4;
				faces[j].rect.y     -= blobs[i].rect.height*0.1;

				faces[j].trackingTimer++;

				if(faces[j].velocity.length() > 100)faces[j].uid = "---"; 
	...
	if (fs-linked>0 && bs-linked==0) { // we have facesn with lost track
		for (int j=0; j<fs; j++) {
			...
			if(t<=0 && !faces[j].recognitionInProcess) {
